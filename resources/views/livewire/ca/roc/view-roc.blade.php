            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        ...
                        <div class="pl-3">
                            <div class="text-base font-semibold">{{ $roc->name }}</div>
                            <div class="font-normal text-gray-500">Type: {{ $roc->type }}</div>
                        </div>  
                    </th>

                    <td class="px-6 py-4">
                        {{ $roc->address }} City: {{ $roc->city }}, State: {{ $roc->state }}
                    </td>

                    <td class="px-6 py-4">
                        <div class="text-base font-semibold">{{ $roc->primary_email }}</div>
                        <div class="font-normal text-gray-500">{{ $roc->secondary_email }}</div>
                    </td>

                    <td class="px-6 py-4">
                        <div class="text-base font-semibold">{{ $roc->primary_phone }}</div>
                        <div class="font-normal text-gray-500">{{ $roc->secondary_phone }}</div>
                    </td>

                    <td class="px-6 py-4">
                        <a href="{{ route('roc.edit',['rocID' => $roc->company_id]) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                            <x-iconEdit /> 
                        </a>
                    </td>
                </tr>
